...
class TTSLossModule(nn.Module):
    ...
    def forward(
        ...
        code_loss_logits, code_loss_labels = self.layer0_generator.build_code_loss_inputs(
            train_output.logits[:aligned_count],
            target_labels[:aligned_count],
        )
        if code_loss_labels.numel():
            code_loss = F.cross_entropy(code_loss_logits, code_loss_labels)
        else:
